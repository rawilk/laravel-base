    ...
    <x-card>
        ...
        <h3 class="text-lg font-medium text-gray-900 flex items-center">
            @if ($this->enabled)
                ...
                <span>{{ __('base::users.profile.two_factor_enabled_status') }}</span>
            @else
                <x-heroicon-o-x-circle class="h-6 w-6 text-red-600 mr-1" />
                <span>{{ __('base::users.profile.two_factor_disabled_status') }}</span>
            @endif
        </h3>

        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('base::users.profile.two_factor_description') }}
            </p>
        </div>
        ...
        <div class="mt-5">
            @unless ($this->enabled)
                ...
                <x-profile.two-factor-trigger variant="blue" action="enableTwoFactorAuthentication" :confirmEnabled="$this->mustConfirmPassword">
                    {{ __('base::users.profile.two_factor_triggers.enable') }}
            ...
            @else
                @if ($showingRecoveryCodes)
                    ...
                    <x-profile.two-factor-trigger variant="white" class="mr-3" action="regenerateRecoveryCodes" :confirmEnabled="$this->mustConfirmPassword">
                        {{ __('base::users.profile.two_factor_triggers.regenerate_recovery_codes') }}
                    </x-profile.two-factor-trigger>
                @else
