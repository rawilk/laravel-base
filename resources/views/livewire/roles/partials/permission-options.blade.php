...
>
    <div class="sm:flex sm:justify-between sm:space-x-2">
        <div class="text-xs">
            {{-- select all --}}
            @if ($canEdit)
                <span class="dark:text-slate-200">{{ __('base::messages.labels.form.make_selection') }}</span>

                <x-blade::button.link x-on:click="selectAllIn($root)">
                    {{ __('base::messages.labels.form.select_all') }}
                </x-blade::button.link>

                <span class="dark:text-slate-200">/</span>
    ...
    @foreach ($permissions as $groupName => $groupedPermissions)
        <div wire:key="perm-group-{{ $loop->index }}"
             ...
             x-cloak
        >
            <div class="bg-slate-50 dark:bg-gray-600 rounded-lg">
                ...
                >
                    <div x-on:click="open = ! open"
                         ...
                         role="button"
                    >
                        <h4 class="capitalize text-sm leading-6 text-slate-900 dark:text-slate-300 font-medium group-hover:text-slate-600 dark:group-hover:text-slate-400 transition-colors flex items-center space-x-1">
                            <span>{{ $groupName }}</span>
                        ...
                        <div class="p-1 h-6 w-6 rounded-full group-hover:bg-slate-400 dark:group-hover:bg-slate-800 transition-all flex justify-center items-center">
                            <x-css-chevron-down
                                class="h-5 w-5 text-slate-600 dark:text-slate-300 group-hover:text-slate-200 dark:group-hover:text-slate-400 transition-all"
                        ...
                        <div class="mt-2 -space-y-px bg-white rounded-md">
                            @foreach ($groupedPermissions as $permission)
                                <div wire:key="perm-{{ $permission->id }}"
                                     ...
                                     x-bind:class="{ 'bg-blue-50 border-blue-200 dark:bg-blue-800 dark:border-blue-600 z-10': has('{{ $permission->id }}'), 'border-slate-200 dark:bg-slate-800 dark:border-slate-600': ! has('{{ $permission->id }}') }"
                                >
                                    <label @class([
                                        'flex p-4',
                                        ...
                                        <div class="ml-3 flex flex-col">
                                            <span class="block text-sm font-medium"
                                                  x-bind:class="{ 'text-blue-900 dark:text-blue-200': has('{{ $permission->id }}'), 'text-slate-900 dark:text-slate-300': ! has('{{ $permission->id }}') }"
                                            >
                                                {{ $permission->name }}
                                            </span>
                                            ...
                                            @if ($permission->description)
                                                <span class="text-sm block"
                                                      x-bind:class="{ 'text-blue-700 dark:text-blue-300': has('{{ $permission->id }}'), 'text-slate-500 dark:text-slate-400': ! has('{{ $permission->id }}') }"
                                                >
